    ...
    if (tile_fade == 1)
    {
        if (cur_alpha != battlealpha)
        {
            for (var i = 0; i < array_length_1d(tilearray); i++)
                tile_set_alpha_ch1(tilearray[i], battlealpha * 2 * max_battlealpha);
        }
    }
...
draw_sprite_ext(spr_ponman_eye_white_ch1, image_index, x + xx, y + yy, image_xscale, image_yscale, image_angle, c_red, image_alpha);
